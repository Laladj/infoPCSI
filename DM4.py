...
def maxi_partiel(L:list[int], i:int)->int:
    """procède de façon recursive"""
  #  assert i < len(L), 'i doit être compris entre 0 et len(L)'
    if len(L[:i]) == 1:
        return L[0]
    else:
        m = maxi_partiel(L, [i-1])
        return m if m > L[0] else L[0]
assert maxi_partiel( [x for x in range(10)], 3 ) == 2

def maxi(L:list)->int:
    """List ->int
        renvoie le maximum de la liste d'entiers passée en argument"""
    pass
...
Supposons que la complexité de la multiplication d'un entier de taille n est
de n log n :
Source : https://www.polytechnique.edu/actualites/complexite-de-la-multiplication-joris-van-der-hoeven-recompense


si l'on augmente la taille de la liste de 1 : la fonction effectue un produit
...
""" def plus_grand_produit_successif(ch, p):
    en_construction = False # True : le produit contient un 0 ; False : le produit de contient pas de 0
    nb_sans_zeros = 0 """
...
def plus_grand_produit_successif(ch, p):
    ...
    nb_sans_zeros = 0  # Nombre de chiffres non nuls consécutifs dans la fenêtre
    en_construction = True  # Indique si le produit est valide ou doit être reconstruit
...
#iv
"""On obtient une meilleure complexité que précédemment, due a la gestion des 0"""
...
# on obtient 0 De fait, dans une chaine de 100 nombres, la loi binomiale nous indique une probabilité
...
#le résultat comporte 656 chiffres. On remarque un grand nombre de zeros à la fin de l'entier
...
def regle1(mot:str)->str:
    """Args : str->str
        transforme la chaine mot se terminant par I en la chaîne mot + ”U” """
...
#regle1("") #AssertionError: La chaîne passée en argument doit être non vide

def regle2(mot:str)->str:
    """Args : str -> str
    prend en argument unique une chaîne de caractères tous égaux à ”M”, ”I” ou ”U”, représentant
    un mot du système MIU, et qui renvoie le mot produit
    par application unique de la règle no 2 si cette règle est applicable, et rien sinon.
    """
    #mot.upper()
    if mot[0] == "M":
        mot += mot[1:]
...
def theoremes_MIU(n:int)->list: #version "sauvage"
    if n<= 1:
        return ['MI']
    else:
        output = ['MI']
        ...
        while len(output) < n+1:
            ...
            output  = list(filter(lambda x:x is not None, output)) #https://www.geeksforgeeks.org/python-remove-none-values-from-list/
