...
def ind_maxi(L:list):
    ...
    Elle rend la position dans la liste comme on l'entend, c'est à dire que l'élément 0 est l'element 1"""
    indice, maximum = -1, -1
    for i, element in enumerate(L):
        if element >= maximum:
            indice, maximum = i, element
        else : pass
    return i
...
def maxi2(L:list):
    ...
    if elementsEgaux:
        return None
    else: L2 = [i for i in x if x != maximumListe(L)]
    return maximumListe(L2)

maxi2([1,3,8,12, 7, 4, 12, 8, 2])